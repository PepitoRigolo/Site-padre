...
class Form {
    ...
    public function inputDateTime (string $key, string $label): string
    {
        $value = $this->getValue($key);
        $type = "datetime-local";
        return <<<HTML
            <div class="form-group">
                <label for="field{$key}">{$label}</label>
                <input type="{$type}" id="field{$key}" class="{$this->getInputClass($key)}" name="{$key}" value="{$value}" step="1800"required>
                {$this->getErrorFeedback($key)}
            </div>
        HTML;
    }
    ...
    public function select(string $key, string $label, array $options = []): string
    {
        ...
        return <<<HTML
            <div class="form-group">
                <label for="field{$key}">{$label}</label>
                <select id="field{$key}" class="{$this->getInputClass($key)}" name="{$key}[]" required multiple>{$optionHTML}</select>
                {$this->getErrorFeedback($key)}
            </div>
        HTML;
    }
